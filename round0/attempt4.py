...
class TomatoTrader(ProductTrader):
    HISTORY_KEY = "tomatoes_mid_history"
    HISTORY_WINDOW = 20
    ENTRY_Z = 1.0
    OPEN_SPREAD = 1
    ...
    def _estimate_fair(self):
        ...
        history = list(self.last_trader_data.get(self.HISTORY_KEY, []))
        history.append(mid)
        ...
        variance = sum((price - mean) ** 2 for price in history) / len(history)
        ...
        # Convert z-score into a fair-value anchor that fades extreme moves back toward the rolling mean.
        fair = mean - (z_score * std * 0.5)

        self.new_trader_data[self.HISTORY_KEY] = history
        self.new_trader_data["tomatoes_zscore"] = z_score
        self.new_trader_data["tomatoes_mean"] = mean
        return fair
