...
class TomatoTrader(ProductTrader):
    ...
    ENTRY_Z = 1.0
    OPEN_SPREAD = 1
    WEAK_SIGNAL_STRENGTH = 0.35
    INVENTORY_SKEW_TICKS = 2.0
    INVENTORY_SIDE_NEUTRAL = 0.3
    ...
    def get_orders(self) -> Dict[str, List[Order]]:
        ...
        if self.position_limit > 0:
            inv_ratio = self.initial_position / self.position_limit
            inv_ratio = max(-1.0, min(1.0, inv_ratio))

        # Reduce adding to crowded inventory side while still allowing unwind on the opposite side.
        buy_mult = max(0.0, min(2.0, 1.0 - inv_ratio))
        sell_mult = max(0.0, min(2.0, 1.0 + inv_ratio))

        take_buy_cap = int(round(self.max_allowed_buy * strength * buy_mult))
        take_sell_cap = int(round(self.max_allowed_sell * strength * sell_mult))
        ...
        inv_skew = inv_ratio * self.INVENTORY_SKEW_TICKS
        quote_bid = int(round(fair - 1 - inv_skew))
        quote_ask = int(round(fair + 1 - inv_skew))
        ...
        if quote_bid < quote_ask:
            ...
            bid_size = int(round(self.max_allowed_buy * passive_scale * buy_mult))
            ask_size = int(round(self.max_allowed_sell * passive_scale * sell_mult))

            # In weak-signal regimes, quote primarily in forecast direction unless inventory needs rebalancing.
            if strength < self.WEAK_SIGNAL_STRENGTH:
                if edge >= 0 and inv_ratio < self.INVENTORY_SIDE_NEUTRAL:
                    ask_size = 0
                elif edge < 0 and inv_ratio > -self.INVENTORY_SIDE_NEUTRAL:
                    bid_size = 0

            if bid_size > 0:
                self.bid(quote_bid, bid_size)
            if ask_size > 0:
                self.ask(quote_ask, ask_size)
