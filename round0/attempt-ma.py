...
class TomatoTrader(ProductTrader):
    ...
    def _estimate_fair(self):
        bid_levels = list(self.buy_orders.keys())
        ask_levels = list(self.sell_orders.keys())

        # Use the average of the two worst levels on each side when available.
        if len(bid_levels) >= 2 and len(ask_levels) >= 2:
            worst_bid_avg = 0.5 * (bid_levels[-1] + bid_levels[-2])
            worst_ask_avg = 0.5 * (ask_levels[-1] + ask_levels[-2])
            mid = 0.5 * (worst_bid_avg + worst_ask_avg)
        elif self.best_bid is not None and self.best_ask is not None:
        ...
        prev = self.last_trader_data.get(self.EMA_KEY)
        if prev is None:
            ema = mid
        else:
            alpha = 2.0 / (self.EMA_WINDOW + 1)
            ema = alpha * mid + (1.0 - alpha) * float(prev)

        self.new_trader_data[self.EMA_KEY] = ema
        return ema
